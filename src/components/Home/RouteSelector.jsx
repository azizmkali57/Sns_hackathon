...
async function searchPlaces(query) {
  ...
  const res = await fetch(url.toString(), {
    headers: {
      "User-Agent": "SafeRouteNavigationSystem/1.0",
    },
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        {fetching && (
          <span className="w-3 h-3 rounded-full border border-white/20 border-t-white/60 animate-spin flex-shrink-0" />
        )}
          ...
          {suggestions.map((place, i) => (
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-[12.5px] text-[#F5F7FA] font-medium truncate">
                  {place.shortLabel}
                </p>
...
export default function RouteSelector({
  ...
  const swap = () => {
    ...
    setSrcCoords(dstCoords);
    setDstCoords(srcCoords);
  ...
  const findRoutes = async () => {
    ...
    try {
      ...
      if (!src) {
        ...
        if (!r.length)
          throw new Error(
            `Could not find "${srcText}" on the map.`
          );
      ...
      if (!dst) {
        ...
        if (!r.length)
          throw new Error(
            `Could not find "${dstText}" on the map.`
          );
      ...
      const apiSrc = data.data.sourceCoords
        ? {
            lat: data.data.sourceCoords.lat,
            lng: data.data.sourceCoords.lng,
          }
        : src;

      const apiDst = data.data.destinationCoords
        ? {
            lat: data.data.destinationCoords.lat,
            lng: data.data.destinationCoords.lng,
          }
        : dst;
  ...
  return (
    <div
      className={`flex flex-col gap-4 font-[Inter,sans-serif] ${className}`}
    >
      <div
        className="
          bg-white/[0.03]
          border border-[#00D1FF]/15
          rounded-2xl
          p-3 sm:p-4 md:p-5
          focus-within:border-[#00D1FF]/35
      ...
      >
        <PlaceInput
          ...
          rightSlot={
            <button
              ...
              className="text-[10px] sm:text-[11px] text-[#00D1FF]/60 hover:text-[#00D1FF] transition-colors flex-shrink-0 font-medium"
            >
              Auto
            </button>
          }
        ...
        <div className="flex items-center justify-center my-3">
          <button
            onClick={swap}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-white/[0.05] border border-white/10 text-white/40 hover:border-[#39D353]/40 hover:text-[#39D353] transition-all"
          >
            <FiRotateCcw size={13} />
          </button>
        </div>
        ...
        <button
          ...
          className="
            ...
            rounded-xl mt-4
            bg-gradient-to-r from-[#39D353] to-[#2ab040]
            text-[#081120]
            font-[Poppins,sans-serif]
            font-bold
            text-[12px] sm:text-[13px] md:text-[14px]
            tracking-wide
            shadow-[0_0_24px_rgba(57,211,83,.30)]
            hover:shadow-[0_0_36px_rgba(57,211,83,.45)]
        ...
        >
          {loading ? (
            <>
              <span className="w-3.5 h-3.5 rounded-full border-2 border-[#081120]/30 border-t-[#081120] animate-spin" />
              Analysing routes…
            </>
          ) : (
            <>
